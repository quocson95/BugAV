...
void OpenGLDisplay::updateData(unsigned char **data)
{
    memcpy(impl->mBufYuv, data, impl->mFrameSize);
    update();
}

void OpenGLDisplay::initShader(int w, int h)
{

    if(impl->mVideoW!=w || impl->mVideoH !=h)
    {
        int size=w*h*3/2;
        impl->mVideoW = w;
        impl->mVideoH = h;
...
//        impl->mBufYuv = new unsigned char[size];
        impl->mBufYuv =(unsigned char*)malloc(size);
    }
}
