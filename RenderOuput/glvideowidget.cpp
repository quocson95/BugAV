...
void GLVideoWidget::updateData(AVFrame *frame)
{
    if (width ==0 ||height ==0) {
        setYUV420pParameters(frame->width, frame->height);
        data = (char*)malloc(frame->linesize[0] * frame->height * 3/2);
    }
    upload_tex = true;
    auto tmp = data;
    memcpy(tmp, frame->data[0],frame->linesize[0] * frame->height);
    tmp += frame->linesize[0] * frame->height;
    memcpy(tmp, frame->data[1],frame->linesize[1] * frame->height/2);
    tmp += frame->linesize[1] * frame->height/2;
    memcpy(tmp, frame->data[2],frame->linesize[2] * frame->height/2);
    plane[0].data = data;
    if (plane.size() > 1) {
        plane[1].data = plane[0].data + plane[0].stride*height;
        plane[2].data = plane[1].data + plane[1].stride*height/2;
...
void GLVideoWidget::setRegionOfInterest(int x, int y, int w, int h)
{

}

QSize GLVideoWidget::videoFrameSize() const
{
    return  QSize(0, 0);
}

void GLVideoWidget::setQuality(int quality)
{

}

void GLVideoWidget::setOutAspectRatioMode(int ratioMode)
{
