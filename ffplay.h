...
static const char* wanted_stream_spec[AVMEDIA_TYPE_NB] = {nullptr};
...
public:
    ...
    static std::vector<TextureFormatEntry> sdl_texture_format_map;

    static ShowModeClock av_sync_type;

    static int screen_width;
    static int screen_height;
    static void init();

    static int cmp_audio_fmts(AVSampleFormat fmt1, int64_t channel_count1,
    ...
    static int packet_queue_put_private(PacketQueue *q, AVPacket *pkt)
    {
        ...
        q->last_pkt = pkt1;
        q->nb_packets++;
        q->size += static_cast<unsigned long>(pkt1->pkt.size) + sizeof(*pkt1);
    ...
    static void video_image_display(VideoState *is)
    {
        ...
        SDL_RenderCopyEx(renderer,
                         ...
                         0,
                         NULL,
                         static_cast<SDL_RendererFlip>(vp->flip_v ? SDL_FLIP_VERTICAL : 0));
...
//        default_height = rect.h;
    ...
    static ShowModeClock get_master_sync_type(VideoState *is) {
        if (is->av_sync_type == int(ShowModeClock::AV_SYNC_VIDEO_MASTER)) {
            if (is->video_st)
                return (ShowModeClock::AV_SYNC_VIDEO_MASTER);
            else
                return (ShowModeClock::AV_SYNC_AUDIO_MASTER);
        } else if (is->av_sync_type == int(ShowModeClock::AV_SYNC_AUDIO_MASTER)) {
            if (is->audio_st)
                return (ShowModeClock::AV_SYNC_AUDIO_MASTER);
            else
    ...
    static double compute_target_delay(double delay, VideoState *is)
    {
        ...
        if (get_master_sync_type(is) != ShowModeClock::AV_SYNC_VIDEO_MASTER) {
    ...
    static void video_refresh(void *opaque, double *remaining_time)
    {
        ...
        if (!is->paused && get_master_sync_type(is) == ShowModeClock::AV_SYNC_EXTERNAL_CLOCK && is->realtime)
    ...
    retry:
            if (frame_queue_nb_remaining(&is->pictq) == 0) {
                ...
                if (frame_queue_nb_remaining(&is->pictq) > 1) {
                    ...
                    if(!is->step && (framedrop>0 || (framedrop && get_master_sync_type(is) != ShowModeClock::AV_SYNC_VIDEO_MASTER)) && time > is->frame_timer + duration){
                        is->frame_drops_late++;
                        frame_queue_next(&is->pictq);
                        goto retry;
                    }
...
//                       is->video_st ? is->viddec.avctx->pts_correction_num_faulty_pts : 0);
    ...
    static int get_video_frame(VideoState *is, AVFrame *frame)
    {
        ...
        if (got_picture) {
            ...
            if (framedrop>0 || (framedrop && get_master_sync_type(is) != ShowModeClock::AV_SYNC_VIDEO_MASTER)) {
                if (frame->pts != AV_NOPTS_VALUE) {
    ...
    static int synchronize_audio(VideoState *is, int nb_samples)
    {
        ...
        if (get_master_sync_type(is) != ShowModeClock::AV_SYNC_AUDIO_MASTER) {
...
//                av_log(NULL, AV_LOG_ERROR, "Stream specifier %s does not match any %s stream\n", wanted_stream_spec[i], av_get_media_type_string(i));
    ...
    static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
    {
        ...
        is->muted = 0;
        is->av_sync_type = int(av_sync_type);
...
//               stream_index);
    ...
    static int opt_show_mode(void *optctx, const char *opt, const char *arg)
    {
        show_mode = !strcmp(arg, "video") ? ShowMode::SHOW_MODE_VIDEO :
                    ...
                    static_cast<ShowMode>(parse_number_or_die(opt, arg, OPT_INT, 0, int(ShowMode::SHOW_MODE_NB)-1));
        return 0;
    }
